...
export async function POST(request: NextRequest) {
  try {
    ...
    const cafeContext = formData.get('cafeContext') as string || '';
    ...
    const relationshipMap: Record<string, string> = {
      bekar: 'bekâr',
      iliski: 'bir ilişkisi olan',
      nisanli: 'nişanlı',
      evli: 'evli',
      karisik: 'karmaşık bir aşk hayatı olan',
    };
    const relationshipText = relationshipMap[relationship] || relationship;

    const prompt = `Sen yüzyıllık bir geleneğin temsilcisi, mistik ve bilge bir kahve falı ustasısın. Sana ${name} isimli, ${genderText} ve ${relationshipText} bir kişinin kahve fincanı/fincanları gösterildi.${cafeContext ? `\n\nBağlam: ${cafeContext}` : ''}
...
💫 **Aşk & Kalp**
Duygusal dünya, aşk ve ilişkiler hakkında özel bir yorum. ${name}'in adını kullan. 2-3 cümle.
...
Türkçe yaz. Gizemli, umut verici ve kişisel bir dil kullan. Samimi ve etkileyici ol. Toplam yaklaşık 220-260 kelime olsun.`;
    ...
    const falText =
      ...
      'Falınız şu an okunamamaktadır. Lütfen tekrar deneyin.';
