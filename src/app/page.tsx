        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              {photos.map((photo, idx) => (
                <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden border border-gold-500/30 group">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button type="button" onClick={() => removePhoto(idx)} className="bg-red-500/80 text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-red-500 transition-colors">
                      &times;
                    </button>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <div className="relative">
              ...
              <input type="text" id="name" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-wood-950/50 border border-gold-600/20 rounded-xl px-5 py-4 text-gold-300 font-inter focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all placeholder-gold-600/30" placeholder="Örn: Ayşe Yılmaz" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="relative">
                ...
                <select id="gender" required value={formData.gender} onChange={(e) => setFormData({...formData, gender: e.target.value})} className="w-full bg-wood-950/50 border border-gold-600/20 rounded-xl px-5 py-4 text-gold-300 font-inter focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all appearance-none">
              ...
              <div className="relative">
                ...
                <select id="relationship" required value={formData.relationship} onChange={(e) => setFormData({...formData, relationship: e.target.value})} className="w-full bg-wood-950/50 border border-gold-600/20 rounded-xl px-5 py-4 text-gold-300 font-inter focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all appearance-none">
                  <option value="" disabled>Seçiniz</option>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full relative overflow-hidden rounded-xl bg-gradient-to-r from-gold-600 via-gold-400 to-gold-600 text-wood-950 font-cinzel font-bold text-lg py-5 transition-all hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(212,175,55,0.3)] disabled:opacity-70 disabled:hover:scale-100 mt-8">
